...
export default function Chimera({ champions }) {
  const defaultTeam = Array.from({length:15}).map((_,i)=>({ slot:i+1, name:'', id:null, base_spd:null, offset:0, tune_ratio:'4:3', boss_type:'Chimera', primary_cd:4 }))
  ...
  function onExport(){
    ...
    const a=document.createElement('a'); a.href=url; a.download='chimera_tune.csv'; a.click(); URL.revokeObjectURL(url)
  }

  return (
    <div>
      <h2 className="text-xl font-semibold">Chimera Tuner</h2>
      <p className="muted">Chimera stage tuning.</p>
