...
export default function Hydra({ champions }) {
  const defaultTeam = Array.from({length:15}).map((_,i)=>({ slot:i+1, name:'', id:null, base_spd:null, offset:0, tune_ratio:'4:3', boss_type:'Hydra', primary_cd:4 }))
  ...
  function onExport(){
    ...
    const a=document.createElement('a'); a.href=url; a.download='hydra_tune.csv'; a.click(); URL.revokeObjectURL(url)
  }

  return (
    <div>
      <h2 className="text-xl font-semibold">Hydra Tuner</h2>
      <p className="muted">Hydra stage tuning. Set reference champion and compute.</p>
